...
def dense_kldiv(logits, targets, T=1.0, reduction='batchmean'):
    q = F.log_softmax(logits / T, dim=1)
    p = F.softmax(targets / T, dim=1)
    '''
    print("p",p)
    print("q",q)
    print("shape",F.kl_div(q, p, reduction=reduction).shape)
    print("div",F.kl_div(q, p, reduction=reduction).sum(1))
    print("T",T*T)
    '''
