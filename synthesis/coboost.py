...
class COBOOSTSynthesizer(BaseSynthesis):
    def __init__(self, teacher, mdl_list, student, generator, nz, num_classes, img_size, save_dir, iterations=30,
        ...
        # generator
        self.generator = generator.cuda(cud).train()
    ...
    def synthesize(self, cur_ep=None):
        ...
        if self.bn == 0:
            self.hooks = []
        best_cost = 1e6
        z = torch.randn(size=(self.synthesis_batch_size, self.nz)).cuda(cud)
        z.requires_grad = True
        targets = torch.randint(low=0, high=self.num_classes, size=(self.synthesis_batch_size,))
        # targets = targets.sort()[0]
        targets = targets.cuda(cud)
        ...
        for it in range(self.iterations):
            ...
            t_out = self.teacher(inputs)#Aw(xs)
            #inputs.retain_grad()
            if len(self.hooks) == 0 or self.bn == 0:
                loss_bn = torch.tensor(0).cuda(cud)
            else:
            ...
            loss_adv = -(dense_kldiv(s_out, t_out,T = 1, reduction='none').sum(1)).mean()  # decision adversarial distillation
            ...
            if it % self.args.print_freq == 0 or it == self.iterations-1:
                self.args.logger.info('[GAN_Train] Iter={iter} L_BN={a_bn:.3f} * {l_bn:.3f}; L_oh={a_oh:.3f} * {l_oh:.3f};'
                                  ' L_adv={a_adv:.3f} * {l_adv:.3f}; LR={lr:.5f};LOSS={LOSS:.5f}'
        ...
        if self.args.weighted and cur_ep != 0 and cur_ep== 0:
            ...
            for w_adjust in range(self.args.wa_steps):
                for idx, (images, labels) in enumerate(self.get_data(labeled=True)):
                    ...
                    tmp_model = WEnsemble(self.mdl_list, mix_weight).cuda(cud)
                    # forward
    ...
    def sample(self):
        if self.args.batchonly == True and self.args.batchused == False:
            self.generator.eval()
            z = torch.randn(size=(self.sample_batch_size, self.nz)).cuda(cud)
    ...
    def get_data(self,labeled=True):
        datasets = self.data_pool.get_dataset(transform=self.transform, labeled=labeled)  
