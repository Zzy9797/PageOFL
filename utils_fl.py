...
def test(model, test_loader,logger):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            data, target = data.cuda(cud), target.cuda(cud)
            output = model(data)
...
def partition_data(trn_dst,tst_dst, partition, beta=0.4, num_users=5,logger=None,args=None):
    ...
    if partition == "iid":
        ...
        while np.sum(clnt_data_list) != 0:
            ...
            middle_curr=cls_priors[curr_clnt] 
            middle_vector=np.arange(n_cls)
            while True:
                for i in range(0,n_cls):
                    middle_par=n_cls-i-1
                    if cls_amount[middle_par]<=0:
                        middle_curr=np.delete(middle_curr,middle_par)
                        middle_vector=np.delete(middle_vector,middle_par)
                middle_curr_sum=middle_curr.sum()
                middle_curr=middle_curr/middle_curr_sum
                middle_cum=np.cumsum(middle_curr,axis=0)
                cls_middle = np.argmax(np.random.uniform() <= middle_cum)
                cls_label=middle_vector[cls_middle]
                
                print("curr",middle_curr)
                print("vector",middle_vector)
                print("cum",middle_cum)
                ...
                if cls_amount[cls_label] <= 0:
                    ...
                    break
        # After data allocation
        for client_id, data_indices in net_dataidx_map.items():
            print(f"Client {client_id} has {len(data_indices)} data samples.")
        ...
        for j in range(n_client):
            net_dataidx_map[j] = []
        idx_dict = {i: idx_list[i] for i in range(len(idx_list))}
        net_dataidx_map = non_iid(idx_dict, n_client, alpha)

    client_data_counts = {client_id: len(data_indices) for client_id, data_indices in net_dataidx_map.items()}


    for client_id, data_count in client_data_counts.items():
        print(f"Client {client_id} has {data_count} samples.")   
...
def get_FL_train_dataloaders(args, domains):
    ...
    for domain in domains:
        data_path = os.path.join('/home/lab-jiang.xingcheng/fedsam/Data/Raw/', args.dataset, 'train',domain)
        train_loader = get_one_train_dataloader(args, data_path, domain)
        train_loaders.append(train_loader)
    return train_loaders

def get_one_train_dataloader(args, data_path, domain):
    domains_list = os.listdir(os.path.join('/home/lab-jiang.xingcheng/fedsam/Data/Raw/', args.dataset, 'all'))
