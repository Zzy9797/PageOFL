...
NORMALIZE_DICT = {
    ...
    'raf': dict(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)),
}
    ...
    if name=='mnist':
        ...
        transform = T.Compose([ T.Resize(64),
            ...
            T.Normalize(mean=[0.5, 0.5, 0.5],
                                 std=[0.5, 0.5, 0.5])])
        root_dir = "/home/lab-jiang.xingcheng/fedsam/Data/Raw/tiny-imagenet-200/"
        ...
        data_root = os.path.join(data_root, 'RAF')  
        

        train_dst = RAFDataset(data_root, transform=train_transform)
        val_dst = RAFDataset(data_root, transform=val_transform)
...
class RAFDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        for label_id in range(7): 
            label_dir = os.path.join(data_dir, 'train', str(label_id))  
        ...
        self.test_labels = []
        for label_id in range(7):
            label_dir = os.path.join(data_dir, 'test', str(label_id))  
            for image_name in os.listdir(label_dir):
                if image_name.endswith('.jpg') or image_name.endswith('.png'):
                    self.test_image_paths.append(os.path.join(label_dir, image_name))
